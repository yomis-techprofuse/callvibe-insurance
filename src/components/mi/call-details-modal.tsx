    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-foreground/40 p-6 backdrop-blur-[2px]">
      ...
      <div className="relative z-10 my-2 w-full max-w-[1040px] overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-pop)]">
        ...
        <div className="flex items-start justify-between gap-4 border-b border-border px-5 py-3.5">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <h2 className="truncate text-[15px] font-semibold tracking-tight">{call.buyer}</h2>
              <Pill tone={intentTone(call.intentLevel)} dot>
                Intent {call.intent} · {call.intentLevel}
              </Pill>
            </div>
            <p className="mt-1 text-[11.5px] text-muted-foreground">
              {call.id} · {call.project} · {call.date} at {call.time} · Advisor {call.advisor}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-wrap items-center gap-1.5 border-b border-border bg-muted/30 px-5 py-2.5">
          ...
          <Pill tone="classify">Project Inquiry</Pill>
          <Pill tone="info">
          ...
          <Pill tone="primary">{call.buyerType}</Pill>
        </div>
        ...
        <div ref={scrollRef} className="scroll-slim max-h-[calc(100vh-190px)] space-y-4 overflow-y-auto bg-canvas p-5">
          ...
          <Panel>
            ...
            <div className="grid gap-3 px-4 py-3 md:grid-cols-2">
              <div className="flex items-center gap-3 rounded-lg border border-border bg-muted/30 px-3 py-2.5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground">Sales Advisor · Marhaba Developments</p>
                </div>
              </div>
              <div className="flex items-center gap-3 rounded-lg border border-border bg-muted/30 px-3 py-2.5">
                <span className="flex h-9 w-9 items-center justify-center rounded-full bg-classify-soft text-[11px] font-semibold text-classify">
                  {call.buyer.split(" ").map((n) => n[0]).slice(0, 2).join("")}
                </span>
                <div className="min-w-0">
                  <p className="text-[12.5px] font-semibold">{call.buyer}</p>
                  <p className="text-[11px] text-muted-foreground">
                    Buyer · {call.phone} · {call.channel === "Call" ? "Voice" : "WhatsApp"}
                  </p>
                </div>
              </div>
            </div>
            <div className="flex flex-wrap gap-x-6 gap-y-1 border-t border-border px-4 py-2.5 text-[11px] text-muted-foreground">
              ...
              <span>Source: <span className="font-medium text-foreground">Marhaba Telephony</span></span>
            ...
            <Panel>
              ...
              <div className="space-y-2 px-4 py-3">
                ...
                <div className="space-y-1.5">
                  {[
                    { t: `Send ${call.config} inventory & floor plans`, d: "Today" },
                    { t: "Send payment plan + service-charge estimate", d: "Today" },
                    { t: call.siteVisit === "Not raised" ? "Propose a site visit slot" : "Confirm Saturday site visit", d: "13 Aug" },
                  ].map((a) => (
          ...
          {/* Buyer intelligence */}
          <Panel>
            <PanelHead title="Buyer Intelligence" icon={<Building2 className="h-3.5 w-3.5" />} subtitle="Extracted automatically from this conversation" />
            <div className="grid gap-2.5 px-4 py-3.5 sm:grid-cols-2 lg:grid-cols-4">
              <Field label="Project" value={call.project} tone="primary" />
              <Field label="Unit Interest" value={call.config} />
              <Field label="Budget" value={call.budget} />
              <Field label="Buyer Type" value={call.buyerType} />
              <Field label="Purchase Timeline" value={call.timeline} />
              <Field label="Financing" value={call.financing} />
              ...
              <Field label="Site Visit" value={call.siteVisit} tone="success" />
              <Field label="Conversation Outcome" value={call.outcome} />
              <Field label="Location Preference" value={call.project.replace("Marhaba ", "")} />
            </div>
          ...
          <Panel>
            ...
            <div className="scroll-slim max-h-[420px] space-y-3 overflow-y-auto px-4 py-3.5">
              {call.transcript.map((t, i) => (
                <div key={i} className={cn("flex gap-2.5", t.speaker === "buyer" && "flex-row-reverse")}>
                  <span
                    className={cn(
